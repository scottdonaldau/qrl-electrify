...
Electron.prototype.package = function(packager_options, done) {
  ...
  //var electronVersion = fs.readFileSync(
  //  join(path.dirname(require('electron')), 'version'), 'utf8')
  //.replace(/^v/, '');
  ...
  if (this.$.env.is_development_mode) {
    args.afterCopy = [
      ...
      function(buildPath, electronVersion, platform, arch, callback) {
        ...
        packageJson.dependencies['qrl-electrify'] = 'file://' + localElectrifyPath;
        fs.writeFileSync(packageJsonPath, JSON.stringify(packageJson, null, 2));

        var electrifyNodeModulesPath = join(buildPath, 'node_modules', 'qrl-electrify');
        ...
        if (fs.existsSync(packageLockPath)) {
          ...
          packageLockJson.dependencies['qrl-electrify'].version = 'file://' + localElectrifyPath;
